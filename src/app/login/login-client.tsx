...
function postLoginPath(user: AuthUser, next?: string | null) {
...
export default function LoginPageClient() {
  ...
  useEffect(() => {
    ...
    if (stored && isStaffRole(stored.role)) {
      router.replace(postLoginPath(stored, next));
    }
  }, [router, next]);
  ...
  async function onEmailLogin(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      router.push(postLoginPath(session.user, next));
      router.refresh();
    } catch (err) {
      const message =
        ...
        <div className="rounded-2xl border border-[#e8ece6] bg-white p-6 shadow-sm md:p-8">
          {user && !isStaffRole(user.role) ? (
            <div className="space-y-4">
              <h1 className="font-heading text-2xl font-bold text-[#1A1A1A]">Signed in</h1>
            ...
            <>
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                Staff login opens the backend dashboard. Customers stay on {siteConfig.shortName}.
              </p>
