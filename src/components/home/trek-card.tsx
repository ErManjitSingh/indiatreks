  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-2xl bg-white shadow-[0_10px_30px_rgba(15,23,42,0.07)] ring-1 ring-black/[0.04] transition duration-300 hover:-translate-y-1 hover:shadow-[0_16px_36px_rgba(15,23,42,0.1)]">
      <div className="relative aspect-[4/3] overflow-hidden">
            <Link href={`/treks/${trek.slug}`} className="absolute inset-0 z-[1]">
              <Image
                src={trek.image}
                alt={trek.name}
                fill
        ...
        {trek.badge ? (
          <span
            className={cn(
              "absolute top-3 left-3 z-[2] rounded-md px-2 py-1 text-[10px] font-extrabold tracking-[0.04em] uppercase shadow-sm",
        ...
        <button
          ...
          aria-pressed={saved}
          className={cn(
            "absolute top-3 right-3 z-[2] inline-flex h-8 w-8 items-center justify-center rounded-full bg-white text-foreground shadow-md transition hover:scale-105",
            saved && "text-red-500",
          )}
      ...
      <div className="flex flex-1 flex-col gap-2 p-3.5 sm:p-4">
        <Link href={`/treks/${trek.slug}`} className="flex min-h-0 flex-1 flex-col gap-2">
          <div className="flex items-start justify-between gap-2">
            <h3 className="min-w-0 font-heading text-[15px] font-bold leading-snug tracking-tight text-[#14201a] transition group-hover:text-primary sm:text-[16px]">
              {trek.name}
            </h3>
